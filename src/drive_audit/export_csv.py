    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for file_info in files:
            row = {
                ...
                'owner_email': file_info.owners[0].get('emailAddress') if file_info.owners else '',
                'last_modifying_user_email': file_info.last_modifying_user.get('emailAddress') if file_info.last_modifying_user else '',
                'size_bytes': file_info.size_bytes if file_info.size_bytes is not None else '',
                ...
                'general_has_link_sharing': format_bool(file_info.access.has_link_sharing),
                'policy_is_under_public_folder': format_bool(file_info.policy.is_under_public_folder),
                'policy_is_public_anyone': format_bool(file_info.policy.is_public_anyone),
                'policy_is_public_by_domain': format_bool(file_info.policy.is_public_by_domain),
                'policy_public_outside_public_folder': format_bool(file_info.policy.public_outside_public_folder)
            }
            writer.writerow(row)
...
def save_permissions_csv(files: List[FileInfo], output_path: str):
    ...
    file_location_map: Dict[str, str] = {file_info.id: file_info.location for file_info in files}
