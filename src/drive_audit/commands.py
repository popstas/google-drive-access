...
from .google_client import get_service, move_file, get_file_permissions, find_child_folder
...
def migrate_contact_folders(
    ...
    for idx, row in enumerate(rows, start=2):
        ...
        if not additional_info:
            error_count += 1
            row["Ссылка на Google Drive папку клиента"] = "error: no additional information"
        ...
        if not old_folder_id:
            error_count += 1
            row["Ссылка на Google Drive папку клиента"] = "error: could not extract folder ID from additional information"
            logger.warning("Row {}: Could not extract folder ID from: {}", idx, additional_info[:100])
            continue
        ...
        try:
            ...
            if not folder_name:
                error_count += 1
                row["Ссылка на Google Drive папку клиента"] = "error: folder name is empty"
                logger.warning("Row {}: Folder name is empty for ID {}", idx, old_folder_id)
                continue
            ...
            if new_folder:
                ...
                if new_folder_id:
                    ...
                    error_count += 1
                    row["Ссылка на Google Drive папку клиента"] = "error: found folder but ID is missing"
                    logger.warning("Row {}: Found folder but ID is missing", idx)
            else:
                error_count += 1
                row["Ссылка на Google Drive папку клиента"] = f"error: folder '{folder_name}' not found in new drive"
                logger.warning(
    ...
    if write_to_contacts and planfix_client:
        ...
        for idx, row in enumerate(rows, start=2):
            folder_link = (row.get("Ссылка на Google Drive папку клиента") or "").strip()
            ...
            if not contact_number:
                logger.warning("Row {}: Skipping Planfix update, missing contact number", idx)
                continue
            
            try:
                planfix_client.update_contact(contact_number, folder_link)
                planfix_updated_count += 1
                logger.info("Row {}: Updated contact {} in Planfix", idx, contact_number)
            except Exception as e:
                planfix_error_count += 1
                logger.error("Row {}: Failed to update contact {} in Planfix: {}", idx, contact_number, e)
