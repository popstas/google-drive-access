...
from .model import FileInfo, DriveConfig, AccessInfo, Permission, PermissionDetails
...
def build_file_tree(files_data: List[Dict[str, Any]], config: DriveConfig) -> List[FileInfo]:
    """
    ...
    if config.root_folder_id == config.drive_id:
        logger.debug(f"Scanning entire drive (root_folder_id == drive_id: {config.drive_id})")
    else:
    ...
    for file_data in files_data:
        ...
        if config.root_folder_id == config.drive_id:
            ...
            if not valid_path:
                ...
                logger.debug(f"Skipping file {file_data.get('name')} (id: {file_id}) - not under root_folder_id {config.root_folder_id}")
                continue
        ...
        for p in permissions_data:
            ...
            for d in p.get('permissionDetails', []):
                p_details.append(PermissionDetails(
        ...
        file_info = FileInfo(
            id=file_id,
            name=name,
            type='shortcut' if is_shortcut else ('folder' if mime_type == 'application/vnd.google-apps.folder' else 'file'),
            mime_type=mime_type,
            ...
            shortcut_target_type='folder' if shortcut_details.get('targetMimeType') == 'application/vnd.google-apps.folder' else ('file' if shortcut_details.get('targetId') else None),
            access=access_info
        )
